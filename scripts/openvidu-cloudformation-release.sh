...
case $OPENVIDU_PROJECT in
  ...
  cloudformation_ov_server)
    ...
    # CF Version
    sed "s/@CF_V@/${CF_VERSION}/" CF-OpenVidu-TEMPLATE.yaml > CF-OpenVidu-v$OPENVIDU_VERSION.yaml
    # OV Version
    sed -i "s/@OV_V@/${OPENVIDU_VERSION}/" CF-OpenVidu-v$OPENVIDU_VERSION.yaml
    # OV Demos Version
    sed -i "s/@OVD_V@/${OVD_VERSION}/" CF-OpenVidu-v$OPENVIDU_VERSION.yaml
    # OV Call Version
    sed -i "s/@OVC_V@/${OVC_VERSION}/" CF-OpenVidu-v$OPENVIDU_VERSION.yaml

    cp CF-OpenVidu-v$OPENVIDU_VERSION.yaml CF-OpenVidu-latest.yaml

    aws s3 cp CF-OpenVidu-v$OPENVIDU_VERSION.yaml s3://aws.openvidu.io --acl public-read 
