...
def insertIntoArray(name, version):
    array = {'name': name,
             'version': version}
    return array

def getDependenciesNode(node, nodeName):
    if node.findall('*'):
        for n in node.findall('*'):
            if nodeName in n.tag:
                return n
        else:
            return getDependenciesNode(n, nodeName)

def getDependencies(rootNode, nodeName):
    dependenciesNode = getDependenciesNode(rootNode, nodeName)
    dependencies = []

    for prop in dependenciesNode.findall('*'):
        version = ''
        artifactId = ''
        ...
        if artifactId and version:
            dependencies.append(insertIntoArray(artifactId, version))
...
def main(args):
    ...
    try:
        opts, args = getopt.getopt(args, 'nv:pih', ['dryrun', 'version', 'parent', 'inplace', 'help'])
    ...
    try:
        bump(pom_xml, next_version, in_place, dry_run, get_parent)
        return True
...
def bump(pom_xml, next_version, in_place, dry_run, get_parent):
    ...
    # Find the project's parent version
    if get_parent:
        parent_version_tag = xml.find("./{*}parent/{*}version")
        parent_version = parent_version_tag.text
        log("Current parent version is %s" % parent_version)

    # Check dependencies
    dependencies = getDependencies(xml.getroot(), 'dependencies')
    for d in dependencies:
        log("Dependency name: %s - Dependency version: %s" % (d['name'],d['version']))
        
    # Calculate the next version, if not specified
    if not next_version and current_version is not None:
