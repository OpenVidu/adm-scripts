...
CONTAINER_GPG_PUBLIC_BLOCK=/root/.gpgpub
...
docker run \
  ...
  $([ -f "$GPG_PUBLIC_BLOCK" ] && echo "-v $GPG_PUBLIC_BLOCK:$CONTAINER_GPG_PUBLIC_BLOCK") \
  ...
  -e "GPG_PUBLIC_BLOCK=$CONTAINER_GPG_PUBLIC_BLOCK" \
